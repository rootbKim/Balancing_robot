...
struct Vect3 {float x=0.; float y=0.; float z=0.;};
class MPU9250_SPI {
    ...
    MagnMode _mMode {MGN_CONT_MEAS2}; 
    MagnBits _mBits { MGN_16BITS};
    SampleRate _srd {SR_100HZ};
    ...
    Vect3 a, g, m;
    float q[4] = {1.0f, 0.0f, 0.0f, 0.0f};      // 사원수 배열
    float roll, pitch, yaw;
    float a12, a22, a31, a32, a33;            // 회전행렬으 계수 
    float magnetic_declination = 8.5;       // Seoul 2019.1.2
    uint32_t lastUpdate = 0;          // keep time
    SPIClass *_spi; 
    uint8_t _csPin;
    uint8_t _intPin;
...
public:
  ...
  private:
    ...
    void initMPU9250()   ;
    void initAK8963()   ;
    static void intService(){ _dataReady=true;   }
    void updateSensors();
